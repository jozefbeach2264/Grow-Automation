...
# Substrings that suggest an AC Infinity BLE device name (case-insensitive).
ACI_NAME_PATTERNS = [
    "aci_v",
    "aci",
    "ctrler",
...
def is_aci_device(name: str) -> bool:
    """Return True if the device name matches known AC Infinity patterns."""
    if not name:
        return False
    lower = name.lower()
...
def aci_badge() -> str:
    return f"{_GREEN}{_BOLD}[ACI]{_RESET}"
